...
For activities, the compact layer should answer “what is happening?” and the expanded layer “what can I do?” Media may persist passively; volume is short; recording and critical status remain reachable. Routine notifications join this flow only when Notchbar owns the notification server; External mode must show unavailable history/actions honestly. Coalesce repeat changes, expire hidden transients, restore from current source state, and never steal focus from a deliberate session. Wi-Fi/Bluetooth/battery use existing Linux services. Brightness is conditional on a real backlight signal; camera/microphone claims require separate evidence.
...
Privacy copy should promise **no telemetry in Notchbar core**, local clipboard/Shelf metadata, and disclosure/control of network-capable features where practical. Weather/IP lookup, Open-Meteo, media artwork URLs, Anthropic usage, remote Digivice imagery and third-party plugins are not covered by a blanket “no network” claim. Plugin permission declarations are static checks and disclosure; enabled third-party QML runs in-process without a sandbox.

...
- **A notification arrives:** When the user selects Notchbar as the notification daemon, it may appear briefly over passive media, enter history, and then return to media without displacing an open launcher or file view. When another daemon owns notifications, Notchbar remains usable and does not pretend to show that daemon's history or actions.

# V1 experience — approved direction, implementation pending
...
- **A notification arrives:** On a fresh first run, Notchbar uses its own notification service if none is already running. If another daemon owns notifications, Notchbar leaves it alone, starts in External/off mode, and explains how to switch to Own after the user disables that daemon. In Own mode, a notification may briefly appear over passive media, enter history, then return to media without displacing an open launcher or file view. External/off does not pretend to show the other daemon's history or actions. The mode can be changed later; migration preserves a k4 user's saved/effective choice when safe, with the same non-destructive conflict rule.
...
Atalaya remains an optional inherited window-preview tool. It starts off on a fresh Notchbar installation until closed/idle resource costs are measured; a migrated k4 user keeps their existing effective on/off state.

## Visual, accessibility and privacy principles

The visual language should be calm, legible and distinct from k4, Droppy and Apple's Dynamic Island. Shared typography, spacing, shape and motion rules keep the dock, surface, settings and modules related. Focus is visible, labels do not depend on color alone, text can scale, and blur never makes content unreadable. Visual effects stop when out of view. Clipboard and Shelf metadata remain local. **The Notchbar core has no telemetry**; inherited network-capable features and in-process plugins are disclosed and user-controllable where practical, not guaranteed offline.
...
The detailed contract and early Shelf feasibility gate are in [V1_SPEC.md](V1_SPEC.md). Implementation still needs explicit approval.

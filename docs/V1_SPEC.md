...
| **EXPERIMENTAL / OPTIONAL INHERITED** | Game, Digivice, GPU-heavy Atalaya previews and other niche modules after individual review. Game/Digivice are default-off in the inherited catalog; Atalaya is currently default-on there. | Fresh Notchbar installs default Atalaya **off** until measured idle CPU, closed-state GPU/live-preview and startup costs are negligible. Migrated k4 users retain their effective Atalaya state, including the inherited default if no explicit override exists. Keep it available; test no crash or collateral regression when enabled. Safety, rights and severe performance problems can block *shipping that payload/default* without authorizing deletion. |
...
| Notification arrives | Fresh first run selects Own when the notification bus name is unowned, or External/off when another daemon owns it; migration keeps safe prior intent. In Own mode, briefly interrupts eligible passive activity and enters history. In External/off, the other daemon owns delivery; Notchbar does not claim mirrored history/actions or stop that daemon. |

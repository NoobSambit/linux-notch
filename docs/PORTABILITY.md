...
Current host code and `K4.Isla.posicion` accept left/right; the reference API docs now list all four values. The source API comment and `Toast/BandaToast.qml` still have top/bottom assumptions, so side placement is **experimental/compatibility-only** for v1. That is a product validation boundary, not evidence that another compositor would support the same geometry or layer protocol.

...
| Rich clipboard and source app | WAYLAND STANDARD/PORTAL data control; [ext-data-control-v1](https://wayland.app/protocols/ext-data-control-v1) supports clipboard managers. The [portal Clipboard API](https://flatpak.github.io/xdg-desktop-portal/docs/doc-org.freedesktop.portal.Clipboard.html) is session-bound, not an always-on universal history feed. | V1 substantially improves text/image preview, search, pin and keyboard UX on current storage. Test file-reference MIME offers before inclusion; source-app attribution is later unless reliable. |
...
| Small Shelf file actions / quick tiles / sharing | COMMON LINUX API local file open/reveal/copy-path helpers for v1; LocalSend or cloud providers require separate installed integrations/network/consent. | V1 small open/reveal/copy-path/remove action foundation. Conversion, rich quick tiles and sharing later. Reject AirDrop/iCloud parity claim. |
